...
start1=time.time()
ds=pd.read_csv('diabetic_data1.csv')
ds=ds.sample(frac=0.05,random_state=1)
...
def query(vec, model, k, max_search_radius):
   ...
   #nearest_neighbors = graphlab.SFrame({'id':candidate_set})
    ...
    nearest_neighbors=nearest_neighbors.sort_values('distance')
    #nearest_neighbors=nearest_neighbors.topk('distance', k, reverse=True)
    nearest_neighbors=nearest_neighbors.iloc[:k]
    ids=nearest_neighbors['id']
    ids=ids[1:]
    selected=ds_original.iloc[list(ids),:]
...
def find_nearest(mat,original):
    my_dataframe=pd.DataFrame(columns=ds_original.columns)
    
    p = Pool(4)
    x = p.map(scalar_wrapper, (mat[i::4] for i in range(4)) )
    
    original1=original[0::4]
    original2=original[1::4]
    original3=original[2::4]
    original4=original[3::4]
    
    original1=original1.reset_index()
    original2=original2.reset_index()
    original3=original3.reset_index()
    original4=original4.reset_index()
    
    original1=original1.drop('index',axis=1)
    original2=original2.drop('index',axis=1)
    original3=original3.drop('index',axis=1)
    original4=original4.drop('index',axis=1)
    
    original1=original1.combine_first(x[0])
    original2=original2.combine_first(x[1])
    original3=original3.combine_first(x[2])
    original4=original4.combine_first(x[3])
    
    my_dataframe=my_dataframe.append(original1,ignore_index=True)
    my_dataframe=my_dataframe.append(original2,ignore_index=True)
    my_dataframe=my_dataframe.append(original3,ignore_index=True)
    my_dataframe=my_dataframe.append(original4,ignore_index=True)
...
ds_original_null=find_nearest(sparseMatrix_null,ds_original_null)
